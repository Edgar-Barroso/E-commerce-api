...
export class StockEntryRepositoryDatabase implements StockEntryRepository{
    ...
    async getByIdItem(idItem: number): Promise<StockEntry[]> {
        ...
        for(const stockEntryData of stockEntriesData){
            stockEntries.push(new StockEntry(stockEntryData.id_item,stockEntryData.operation,stockEntryData.quantity,stockEntryData.date))
        }
        return stockEntries
    }

    async save(stockEntry: StockEntry): Promise<void> {
        await this.connection.query("insert into ccca.stock_entry (id_item,operation,quantity,date) values ($1,$2,$3,$4)",[stockEntry.idItem,stockEntry.oparation,stockEntry.quantity,stockEntry.date])
    }
